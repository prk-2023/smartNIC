...
# ============================================================================
# 04-run_test_virtio_dualvm.sh
# Same metrics/methodology as the OVS-DPDK run, adapted for the pure-virtio
# (Linux bridge + vhost-net) dataplane - no OVS/PMD stats exist here, so we
# instead capture vhost-net kernel thread placement and physical-NIC kernel
# counters as the equivalent "is the dataplane doing what we expect" check.
# ============================================================================

RUN="${1:?Usage: $0 <run_name>   e.g. pure_virtio_dualvm_run1}"
...
DURATION=60
# Window extended to 145s to protect metric logging timeline safely
MONITOR_DURATION=145
OUTDIR="./results/${RUN}"
mkdir -p "${OUTDIR}"

echo "===== Pure-virtio (Linux bridge + vhost-net) dual-VM run: ${RUN} ====="
...
# ---- vhost-net thread placement + physical NIC kernel counters, before ----
# (equivalent verification to OVS's pmd-stats-show: confirms vhost-net is
# actually engaged and pinned to the expected isolated cores, and that
# traffic really is traversing the kernel driver on the physical NICs)
ps -eLo pid,tid,psr,comm 2>/dev/null | grep 'vhost-' > "${OUTDIR}/vhost_threads_before.txt" || echo "no vhost-net threads found yet" > "${OUTDIR}/vhost_threads_before.txt"
for PF in enp1s0f0np0 enp1s0f1np1; do
    ethtool -S "${PF}" 2>/dev/null | grep -E "^\s*rx_packets:|^\s*tx_packets:" > "${OUTDIR}/ethtool_${PF}_before.txt" || true
done
...
# ---- vhost-net thread placement + physical NIC kernel counters, after ----
ps -eLo pid,tid,psr,comm 2>/dev/null | grep 'vhost-' > "${OUTDIR}/vhost_threads_after.txt" || echo "no vhost-net threads found" > "${OUTDIR}/vhost_threads_after.txt"
for PF in enp1s0f0np0 enp1s0f1np1; do
    ethtool -S "${PF}" 2>/dev/null | grep -E "^\s*rx_packets:|^\s*tx_packets:" > "${OUTDIR}/ethtool_${PF}_after.txt" || true
done
...
summary['note'] = ("This is the pure-virtio (Linux bridge + vhost-net) run - no OVS/DPDK involved. "
                    "host_cpu_busy_pct includes vhost-net kernel worker threads, which only consume "
                    "CPU while actively forwarding packets (unlike DPDK PMD threads, which busy-poll "
                    "at ~100% even when idle). Check vhost_threads_before/after.txt for kernel thread "
                    "placement (psr column = host core) and ethtool_*_before/after.txt to confirm "
                    "rx/tx_packets climbed on the physical NICs during the run.")
