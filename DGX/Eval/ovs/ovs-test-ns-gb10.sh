...
delete_bridge()
{
    ...
    if ovs-vsctl br-exists "${bridge}"; then
        ...
        ovs-vsctl --no-wait del-br "${bridge}"
...
delete_bridge_new()
{
    if ovs-vsctl br-exists br-left; then
        log_info "Deleting OVS bridge: br-left"
        ovs-vsctl --no-wait del-br br-left 
        log_ok "Deleted br-left"
    else
        log_info "OVS bridge br-left does not exist"
    fi

    if ovs-vsctl br-exists br-right; then
        log_info "Deleting OVS bridge: br-right"
        ovs-vsctl --no-wait del-br br-right 
        log_ok "Deleted br-right"
    else
        log_info "OVS bridge br-right does not exist"
    fi
}
...
add_ovs_port()
{
    ...
    ovs-vsctl --no-wait --may-exist add-port "${bridge}" "${interface}"
...
configure_ovs_flows()
{
    ...
    ovs-ofctl --no-wait del-flows "${LEFT_BR}" 2>/dev/null || true
    ovs-ofctl --no-wait del-flows "${RIGHT_BR}" 2>/dev/null || true
