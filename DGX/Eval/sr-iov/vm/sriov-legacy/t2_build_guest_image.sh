#!/usr/bin/env bash
# t2_build_guest_image.sh <1|2> — Generates guest QCOW2 image and cloud-init ISO with Mellanox driver tools
...
# Check for host SSH key to inject
SSH_KEY_FILE=""
if [[ -f "${HOME}/.ssh/id_rsa.pub" ]]; then
    ...
    echo "No existing SSH key found in ~/.ssh/. Generating one..."
...
echo "=========================================================="
echo " Building Image for VM${VM_NUM} (${HOSTNAME})"
echo "=========================================================="

if [[ ! -f "$BASE_IMG" ]]; then
    echo "[1/4] Pulling base Fedora image: ${BASE_IMAGE}"
...
# Remove existing QCOW2 overlay so cloud-init runs fresh
if [[ -f "${OVERLAY_QCOW2}" ]]; then
    echo "Removing existing QCOW2 overlay ${OVERLAY_QCOW2} to apply cloud-init clean..."
    rm -f "${OVERLAY_QCOW2}"
fi
...
runcmd:
  # Enable SSH password auth explicitly in Fedora sshd configs
  ...
  # Enable hardware offloads inside guest for mlx5 VF driver
...
# Guest network config: Match passthrough VF by MAC address
