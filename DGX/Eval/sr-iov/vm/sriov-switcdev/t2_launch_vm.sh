#!/usr/bin/env bash
# t2_launch_vm.sh <1|2> — Launches guest QEMU instance passing SR-IOV VF directly via vfio-pci
...
[[ -f "${VF_BDF_FILE}" ]] || { echo "ERROR: ${VF_BDF_FILE} missing. Run t2_host_setup-sriov_switchdev.sh first."; exit 1; }
...
echo "   CPU Cores : ${VCPU_CORES}"
echo "   Passthrough VF BDF : ${VF_BDF}"
echo "=========================================================="
...
echo "  Management SSH: ssh -p ${SSH_PORT} ${GUEST_USER}@localhost"
