...
#   ens-test: vhost-net tap into br-left (VM1) or br-right (VM2) — the benchmarked path
#   ens-mgmt: QEMU SLIRP (user-mode networking) — internet + SSH, not part of the benchmark
# Run t2_host_setup.sh and t2_build_guest_image.sh for this VM_NUM first.
#
...
ip link show "${TEST_TAP}" &>/dev/null || { echo "ERROR: ${TEST_TAP} missing - run t2_host_setup.sh first"; exit 1; }
...
qemu-system-aarch64 \
  ...
  -netdev tap,id=net0,ifname=${TEST_TAP},script=no,downscript=no,vhost=on,queues=${VM_VCPUS} \
  -device virtio-net-pci,netdev=net0,mq=on,vectors=$((2*VM_VCPUS+2)),disable-legacy=on,mac=${TEST_MAC} \
...
echo "NOTE: run pin_vhost.sh (write one following the taskset pattern above, matching"
echo "      'vhost-<pid>' process names) AFTER traffic starts - vhost threads spawn lazily."
